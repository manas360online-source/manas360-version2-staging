...
export default function TherapistModeGate({ requiredMode, title, description }: TherapistModeGateProps) {
  const { setDashboardMode } = useProviderDashboardContext();
  ...
  return (
    <TherapistCard className="p-5">
      ...
      <div className="mt-4">
        <TherapistButton onClick={() => setDashboardMode(requiredMode)}>
          Switch to {requiredMode === 'professional' ? 'Professional Mode' : 'Self Mode'}
        </TherapistButton>
      </div>
    </TherapistCard>
  );
