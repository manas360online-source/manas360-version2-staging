...
export default function CareTeamPage() {
  ...
  const fetchCareTeamData = async () => {
    try {
      ...
      const providersRes = await patientApi.getAvailableProviders().catch(() => null);
      if (providersRes) {
  ...
  const filteredProviders = useMemo(() => {
    return providers.filter((provider) => {
      ...
      return matchesSearch && matchesSpec && matchesPrice && matchesLanguage;
    });
  }, [providers, searchQuery, selectedSpecialization, maxPrice, selectedLanguage]);
          ...
          providers.map((provider) => (
            <div key={provider.id} className="rounded-lg border border-calm-sage/15 bg-white/90 p-4 hover:border-teal-200 hover:shadow-md transition-all">
              ...
              <div className="flex gap-2">
                ...
                <Link
                  to={`/patient/book-session?providerId=${provider.id}`}
